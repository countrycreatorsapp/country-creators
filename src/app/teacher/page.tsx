...
// Mock Data for the Teacher Leaderboard
const mockStudents = [
...
export default function TeacherCommandCenter() {
  ...
  const handleAward = (e) => {
    ...
    // In production, this fires to Supabase to update the student's resources instantly
  };
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-900 font-sans p-8">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        {/* Left Column: Instant Awards (The Grading Bridge) */}
        <div className="col-span-1 space-y-8">
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
            ...
            <form onSubmit={handleAward} className="space-y-4">
              ...
              <div>
                ...
                <input 
                  type="text" 
                  placeholder="e.g. Aced the Japan Vocab Quiz" 
          ...
          <div className="bg-rose-50 p-6 rounded-xl shadow-sm border border-rose-200">
            ...
            <p className="text-sm text-rose-700 mb-4">Warning: These buttons instantly alter the economy for every student.</p>
            
            <div className="space-y-3">
              <button 
                onClick={() => triggerGlobalEvent('The Archipelago Shift')}
                className="w-full bg-rose-600 hover:bg-rose-700 text-white font-bold py-2 px-4 rounded-lg flex justify-between items-center transition-colors shadow-sm"
              >
                <span>Trigger: Archipelago Shift (Japan)</span>
                <span>🌋</span>
              </button>
              <button 
                onClick={() => triggerGlobalEvent('Golden Age')}
                className="w-full bg-amber-500 hover:bg-amber-600 text-white font-bold py-2 px-4 rounded-lg flex justify-between items-center transition-colors shadow-sm"
              >
                <span>Trigger: Golden Age (Double Prod)</span>
                <span>✨</span>
              </button>
              <button 
                onClick={() => triggerGlobalEvent('Market Crash')}
                className="w-full bg-slate-800 hover:bg-slate-900 text-white font-bold py-2 px-4 rounded-lg flex justify-between items-center transition-colors shadow-sm"
              >
                <span>Trigger: Market Crash (Economics)</span>
                <span>📉</span>
              </button>
            </div>
          </div>
        ...
        {/* Right Columns: The Anonymous Leaderboard */}
        <div className="col-span-2">
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 h-full">
            <div className="flex justify-between items-center mb-6">
              ...
              <button className="text-sm text-indigo-600 font-bold hover:underline">Export to CSV</button>
            </div>
